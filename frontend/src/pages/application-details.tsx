                ...
                <CardContent className="space-y-2">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                    <div className="space-y-2">
                      ...
                      {student ? (
                        <Button variant="link" className="h-8 p-0 text-xs" onClick={() => { try { const { openStudentProfile } = require('@/lib/utils'); openStudentProfile(student.id, setLocation); } catch { try { setLocation(`/students?studentId=${student.id}`); } catch {} } }}>
                          {student.name}
                        </Button>
                      ) : (
