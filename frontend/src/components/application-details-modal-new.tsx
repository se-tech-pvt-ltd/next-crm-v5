                    ...
                    <CardHeader className="pb-3">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-3">
                          {admissionForApp ? (
                            <Button variant="outline" size="sm" className="rounded-full px-2 md:px-3 [&_svg]:size-5" onClick={() => { const detail = { admissionId: admissionForApp.id, admission: admissionForApp }; try { const { useModalManager } = require('@/contexts/ModalManagerContext'); const { openModal } = useModalManager(); openModal(() => window.dispatchEvent(new CustomEvent('openAdmissionDetails', { detail })) ); } catch { onOpenChange(false); setTimeout(() => window.dispatchEvent(new CustomEvent('openAdmissionDetails', { detail })), 160); } }} title="View Admission">
                              <ExternalLink />
