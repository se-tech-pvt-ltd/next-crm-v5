      ...
      <DetailsDialogLayout
        ...
        leftContent={(
          <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative phone-compact">
                      ...
                      {/* WhatsApp quick-chat button (visible when not editing and phone present) */}
                      {!isEditing && String(displayData.phone || '').trim() !== '' && (
                        <a
                          href={(() => {
                            try {
                              // Extract digits and ensure international format without leading +
                              const raw = String(displayData.phone || '');
                              const digits = raw.replace(/[^0-9+]/g, '');
                              // If starts with +, remove it for WhatsApp URL
                          ...
                          className="absolute right-2 top-1.5 inline-flex items-center justify-center h-6 w-6 rounded text-green-600 hover:bg-green-50"
            ...
            <CollapsibleCard defaultOpen={false} persistKey={`lead-details:modal:${lead.id}:lead-access`} header={<CardTitle className="flex items-center space-x-2"><Users className="w-4 h-4 text-primary" /><span>Lead Access</span></CardTitle>} cardClassName="shadow-md border border-gray-200 bg-white">
